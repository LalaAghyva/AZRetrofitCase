...
class SearchListAdapter : RecyclerView.Adapter<SearchListViewHolder>() {
    ...
    companion object {
        private val diffCallBack = object : DiffUtil.ItemCallback<FlightListUIModel>() {
            override fun areItemsTheSame(oldItem: FlightListUIModel, newItem: FlightListUIModel,
            ): Boolean {
                return  oldItem.enuid == newItem.enuid
            }

            override fun areContentsTheSame(oldItem: FlightListUIModel, newItem: FlightListUIModel,
            ): Boolean {
